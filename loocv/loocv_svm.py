...
def loocv_csv(training, testing, loocv_subject):
    ...
    clf = SVC(kernel='linear') ##test for linear/other kernel models
    ...
    plot_confusion_matrix(cnf_matrix, classes=class_names,
                          title='LOOCV %s SVM confusion matrix, without normalization'%(loocv_subject))

    # Plot normalized confusion matrix
    plt.figure()
    plot_confusion_matrix(cnf_matrix, classes=class_names, normalize=True,
                          title='LOOCV %s SVM Normalized Confusion Matrix'%(loocv_subject))
